...
RSpec.describe Request, type: :model do
  ...
  describe "requester_email" do
    context "when the email address is not in a valid format" do
      it "is not valid" do
        %w[user@-example.com user@example-.com invalid@@example.com].each do |invalid_email|
  ...
  describe "deadline" do
    context "when blank" do
      ...
      context "when deadline is not a datetime" do
        ...
        it "does not also add a comparison error" do
          record = FactoryBot.build(:request, deadline: 2_345_6)

          expect(record).not_to be_valid
          expect(record.errors[:deadline]).to eq(["must be a valid datetime"])
        end
      end
      ...
      context "when in the past" do
        it "is not valid" do
          ...
          expect(record).not_to be_valid
          expect(record.errors.full_messages).to include("Deadline must be a date between today and 10 years in the future")
        end
      end

      context "when yesterday's date" do
        it "is not valid" do
          record = FactoryBot.build(:request, deadline: Date.yesterday.iso8601)
    ...
    context "if is today's date without a time" do
      it "should be valid" do
        record = FactoryBot.build(:request, deadline: Date.current.iso8601)
  ...
  describe "source_url" do
    ...
    it "is invalid when it does not have a valid URL format" do
      ["www.gov.uk",
       ...
       "://example.com",
       "https//example.com",
       "not a url https://www.gov.uk",
       "javascript:alert(1)//https://www.gov.uk",
       "https://www.gov.uk\n<script>"].each do |invalid_url|
        record = FactoryBot.build(:request, deadline: 1.year.from_now, source_url: invalid_url)

        expect(record).not_to be_valid
        expect(record.errors.full_messages).to include("Source URL must be a valid URL (http: or https:)")
      end
    end
